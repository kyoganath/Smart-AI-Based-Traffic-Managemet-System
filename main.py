...
# Initial traffic signal timing in seconds
default_signal_time = 90
...
while cap.isOpened():
    ...
    if ret:
        ...
        if cnt_up + cnt_down < 10:
            signal_time = default_signal_time / 2
        elif cnt_up + cnt_down <= 50:
            signal_time = default_signal_time - 30
        else:
            signal_time = default_signal_time

        # Print the current signal time to the terminal
        sys.stdout.write(f"\rCurrent counts - UP: {cnt_up}, DOWN: {cnt_down}, Signal Time: {signal_time} seconds")
        sys.stdout.flush()
